// ============================================
// HELTEC TRACKER V1.2
// Alur: SIM900A → ThingSpeak (HTTP POST JSON)
//       ThingSpeak → Vercel 
// ============================================
...
// Server lama (tetap ada, tidak dihapus)
...
// ============================================
// TFT — 5 halaman (tambah halaman ThingSpeak)
// ============================================
#define TFT_W       160
#define TFT_H       80
#define TFT_PAGES   5
...
TinyGsmClient gsmClientTS(modem);  // client khusus ThingSpeak
#endif
...
// RTOS
// ============================================
SemaphoreHandle_t mGps, mLora, mTft, mWifi, mGprs, mIr, mSpi, mTs;
QueueHandle_t     qLog;
...
void sendThingSpeakGprs();          // ← kirim ke ThingSpeak via SIM900A
...
void setup()
{
  ...
  Serial.begin(115200);
  Serial.println("\n[BOOT] Heltec Tracker V1.3 (SIM900A→ThingSpeak)");
  ...
  tft.setCursor(10, 15); tft.print("Heltec Tracker V1.3");
...
void taskTft(void *pv)
{
  ...
  for (;;)
  {
    ...
    if (xSemaphoreTake(mTft, pdMS_TO_TICKS(100)) == pdTRUE) {
      ...
      fb.print("T:");
      fb.setTextColor(g_ts.lastOk ? ST7735_GREEN : ST7735_RED);
      fb.print(g_ts.lastOk ? "OK" : "--");
      fb.setTextColor(ST7735_WHITE); fb.print(" ");
...
// ============================================
// WIFI TASK (tetap ada, tapi hanya aktif kalau
// hotspot dinyalakan. Kalau tidak dinyalakan,
// WiFi.status() tidak akan pernah WL_CONNECTED
// sehingga tidak akan mengirim apapun)
// ============================================
#if ENABLE_WIFI
void taskWifi(void *pv)
{
  ...
  for (;;) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      // Reconnect otomatis setiap 30 detik kalau hotspot dinyalakan nanti
    ...
    // Kirim ke Vercel via WiFi hanya kalau WiFi tersambung
...
// ============================================
// GPRS — KIRIM KE THINGSPEAK via SIM900A
// HTTP POST JSON ke api.thingspeak.com
// ============================================
#if ENABLE_GPRS
...
void sendThingSpeakGprs()
{
  ...
  if (xSemaphoreTake(mGps, MTX) == pdTRUE) {
    lat = g_gps.lat; lng = g_gps.lng; alt = g_gps.alt; sat = g_gps.sat;
    xSemaphoreGive(mGps);
  }
  if (xSemaphoreTake(mLora, MTX) == pdTRUE) {
    rssi = g_lora.rssi; snr = g_lora.snr;
    xSemaphoreGive(mLora);
  }
  if (xSemaphoreTake(mIr, MTX) == pdTRUE) {
    hitCount = g_ir.hitCount;
    xSemaphoreGive(mIr);
  }

  // Build JSON body untuk ThingSpeak
  // ThingSpeak menerima POST JSON dengan format:
  // {"api_key":"xxx","field1":val,...}
  char jsonBody[300];
  snprintf(jsonBody, sizeof(jsonBody),
    ...
    "\"field4\":%u,"     // satellites
    "\"field5\":%d,"     // rssi
    "\"field6\":%lu,"    // ir hit count
    "\"field7\":%u,"     // cheat detected
    "\"field8\":%.1f}",  // snr
    TS_WRITE_KEY,
    lat, lng, alt,
    sat,
  ...
  if (gsmClientTS.connect(TS_HOST, TS_PORT)) {
    // HTTP POST manual via TinyGsmClient
    ...
    while (gsmClientTS.available() == 0) {
      if (millis() - timeout > 8000) {
        Serial.println("[TS] Timeout waiting response");
        break;
      }
      vTaskDelay(pdMS_TO_TICKS(10));
    }

    // Baca response — ambil HTTP status code
    // ThingSpeak mengembalikan entry_id sebagai body kalau sukses
    // Contoh response: HTTP/1.1 200 OK ... body: {"channel_id":xxx,"entry_id":yyy,...}
    String response = "";
    while (gsmClientTS.available()) {
      response += (char)gsmClientTS.read();
    }
    gsmClientTS.stop();

    // Parse status code dari HTTP response line
    // Format: "HTTP/1.1 200 OK"
    if (response.startsWith("HTTP/")) {
      int spaceIdx = response.indexOf(' ');
      if (spaceIdx > 0) {
        respCode = response.substring(spaceIdx + 1, spaceIdx + 4).toInt();
      }
    }

    // Sukses jika code 200 dan body mengandung entry_id
  ...
  if (xSemaphoreTake(mTs, MTX) == pdTRUE) {
    g_ts.lastOk     = ok;
    g_ts.lastCode   = respCode;
    g_ts.lastSentMs = millis();
    if (ok) g_ts.okCount++; else g_ts.failCount++;
    xSemaphoreGive(mTs);
  }

  char logMsg[80];
  snprintf(logMsg, 80, "[TS] %s code:%d lat:%.4f lng:%.4f hits:%lu",
    ok ? "OK" : "FAIL", respCode, lat, lng, hitCount);
  logSd(logMsg);
...
void buildDataPayload(Payload &payload)
{
  ...
  if (xSemaphoreTake(mGps, MTX) == pdTRUE) {
    payload.lat = g_gps.lat; payload.lng = g_gps.lng; payload.alt = g_gps.alt;
    xSemaphoreGive(mGps);
...
void taskGprsLoop(void *pv)
{
  ...
  uint32_t lastTsSend  = 0;   // timer kirim ke ThingSpeak
  uint32_t lastDbSend  = 0;   // timer kirim ke server lama (opsional)

  for (;;) {
    // Cek & reconnect kalau putus
    ...
    // ─── KIRIM KE THINGSPEAK via SIM900A setiap TS_INTERVAL_MS ───
    if (millis() - lastTsSend >= TS_INTERVAL_MS) {
      sendThingSpeakGprs();
      lastTsSend = millis();
    }

    // ─── KIRIM KE SERVER LAMA setiap GPRS_INTERVAL_MS (opsional) ───
...
void sendImmediateGprsPayload(bool isCheat)
{
  ...
  if (gsmClient.connect(GPRS_HOST, GPRS_PORT)) {
    ...
    if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { currentIp = g_gprs.ip; xSemaphoreGive(mGprs); }
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      g_pg[2].row[2] = "IP: " + currentIp; g_pg[2].row[5] = "Evt: " + event;
      xSemaphoreGive(mTft);
    }
...
void taskIr(void *pv)
{
  for (;;) {
    ...
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      if (xSemaphoreTake(mIr, MTX) == pdTRUE) {
        if (!g_ir.got) {
          g_pg[3].row[1] = "Shooter: -"; g_pg[3].row[2] = "Sub: -";
          g_pg[3].row[3] = "Loc: NONE";  g_pg[3].row[4] = "Time: --:--";
        }
        xSemaphoreGive(mIr);
      }
      xSemaphoreGive(mTft);
