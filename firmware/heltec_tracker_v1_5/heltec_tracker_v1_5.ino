// ============================================
// HELTEC TRACKER V1.5
// Base : V1.2 (program yang GPRS+LoRa jalan)
// Perubahan dari V1.2:
// 1. field6 ThingSpeak: irStatus 1=HIT / 0=NONE
// 2. Status bar TFT: L= G= W= S= (SD, bukan ThingSpeak)
// 3. Serial SD: "SDcard = Connected ...Mb"
// 4. GPS CSV Logger ke /gps_log.csv (core 0)
// 5. Tambah mCsv, CsvSt, taskGpsCsv
// Semua kode lain TIDAK DIUBAH dari V1.2
// ============================================
...
#include <RadioLib.h>
#include <TinyGPS++.h>      // ← tambah untuk GPS CSV
...
// field5=rssi, field6=irStatus(1=HIT/0=NONE) ← UBAH dari hitCount
...
struct CsvSt { uint32_t rowCount; bool headerOk; };  // ← BARU
...
// RTOS
// ============================================
SemaphoreHandle_t mGps, mLora, mTft, mWifi, mGprs, mIr, mSpi, mTs, mCsv;  // ← tambah mCsv
QueueHandle_t     qLog;
...
void writeCsvHeader();             // ← BARU
void writeGpsCsv();                // ← BARU
bool sdUpload();
String decodeState(int16_t);
void initGprsHardware();

// ============================================
// SETUP — sama persis V1.2, tambah mCsv + taskGpsCsv
// ============================================
void setup()
{
  ...
  tft.setCursor(10, 15); tft.print("Heltec Tracker V1.5");
  ...
  mCsv  = xSemaphoreCreateMutex();  // ← BARU
  qLog  = xQueueCreate(LOG_QUEUE_SIZE, sizeof(char*));

  // Urutan task SAMA PERSIS seperti V1.2
  ...
  // ← BARU: GPS CSV di core 0, prioritas rendah, SETELAH semua task lain
...
// ============================================
// TFT TASK — UBAH status bar L= G= W= S=
// ============================================
...
// ============================================
// ANTI-CHEAT TASK — SAMA PERSIS V1.2
// ============================================
...
// ============================================
// GPS CSV TASK ← BARU (core 0)
// Simpan GPS ke /gps_log.csv setiap 5 detik
// File tidak dihapus saat reboot
// ============================================
void writeCsvHeader()
{
  if (!sdOK) return;
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    if (f) {
      if (!exists)
        f.println("millis,jam,menit,detik,latitude,longitude,altitude_m,satellites,gps_valid");
      f.close();
...
void writeGpsCsv()
{
  ...
  int8_t dh = h;
  if (locValid) {
    int utcOffset = (lng < 115.0f) ? 7 : (lng < 134.0f) ? 8 : 9;
    dh = (int8_t)((h + utcOffset + 24) % 24);
  }

  char row[120];
  snprintf(row, sizeof(row), "%lu,%02d,%02d,%02d,%.6f,%.6f,%.1f,%u,%s",
    millis(), dh, m, s, lat, lng, alt, sat, locValid ? "1" : "0");
...
// UBAH: field6 = irStatus (1=HIT, 0=NONE)
//       sebelumnya hitCount
// ============================================
#if ENABLE_GPRS
...
void sendThingSpeakGprs()
{
  ...
  uint8_t irFlag = 0;  // ← UBAH: 1=HIT, 0=NONE (bukan hitCount)
  ...
  if (xSemaphoreTake(mLora, MTX) == pdTRUE) { rssi = g_lora.rssi; snr = g_lora.snr; xSemaphoreGive(mLora); }
  if (xSemaphoreTake(mIr,   MTX) == pdTRUE) { irFlag = g_ir.got ? 1 : 0; xSemaphoreGive(mIr); }  // ← UBAH
  ...
  snprintf(jsonBody, sizeof(jsonBody),
    ...
    "\"field4\":%u,"
    "\"field5\":%d,"
    "\"field6\":%u,"    // ← irStatus: 1=HIT, 0=NONE
    "\"field7\":%u,"
    "\"field8\":%.1f}",
...
// initGprsHardware — SAMA PERSIS V1.2
void initGprsHardware()
{
...
void taskGprsLoop(void *pv)
{
  ...
  for (;;) {
    ...
    if (millis() - lastDbSend >= GPRS_INTERVAL_MS) {
      ...
      Serial.println("[GPRS] Server lama: " + event);
...
// ============================================
// SD TASK — UBAH serial print saja
// ============================================
#if ENABLE_SD
void taskSd(void *pv)
{
  ...
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    while (f.available()) {
      ...
      char pl[400]; snprintf(pl, 400,
        "{\"source\":\"wifi_sd\",\"id\":\"%s\",\"lat\":%.6f,\"lng\":%.6f,\"alt\":%.1f,\"irStatus\":\"%s\",\"battery\":%u,\"satellites\":%u,\"rssi\":%d,\"snr\":%.1f}",
