...
bool sendToSupabase(double lat, double lng) {
  ...
  // Set URL with apikey as query parameter (more reliable than headers on SIM900A)
  String url = String(SUPABASE_URL) + "/rest/v1/locations?apikey=" + String(SUPABASE_KEY);
  ...
  // CRITICAL FIX: Use only Authorization header (apikey is redundant)
  // SIM900A has limited USERDATA buffer, so we send only the essential header
  String authHeader = "Authorization: Bearer " + String(SUPABASE_KEY);
  String headerCmd = "AT+HTTPPARA=\"USERDATA\",\"" + authHeader + "\"";
  
  Serial.println("[HTTP] Setting Authorization header");
  
  if (!sendATCommand(headerCmd.c_str(), 3000)) {
    Serial.println("[ERROR] Failed to set header");
    sendATCommand("AT+HTTPTERM", 2000);
    return false;
  }
