// ============================================
// HELTEC TRACKER V1.1 (FULL + CHEAT BANNER)
// Fix: Status Bar, GPRS Display, Cheat Banner
// ============================================
  ...
  for (;;)
  {
    ...
    if (xSemaphoreTake(mTft, pdMS_TO_TICKS(100)) == pdTRUE) {
      ...
      for (int i = 0; i < TFT_ROWS; i++) {
        ...
        if (y >= TFT_H - 10) break; // Sisakan ruang untuk cheat banner
        fb.setCursor(LEFT_M, y);
        fb.print(g_pg[currentPage].row[i]);
      }

      // === CHEAT RECORDED BANNER ===
      bool showCheatBanner = (millis() < g_cheatAlertExpiry);
      if (showCheatBanner) {
        // Banner merah di bagian bawah
        ...
        // Juga update status GPRS page
        if (currentPage == 2) {
          fb.setCursor(LEFT_M, TOP_M + 5 * LINE_H);
          fb.print("Evt: CHEAT!");
        }
      }
...
void taskGprsLoop(void *pv)
{
  ...
  for (;;) {
    ...
    if (millis() - lastScheduledSend >= GPRS_INTERVAL_MS) {
      ...
      // >>> UPDATE TFT DISPLAY (GPRS/DB Status) <<<
      if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
        ...
        g_pg[2].row[3] = "Last: " + String(millis()/1000);
