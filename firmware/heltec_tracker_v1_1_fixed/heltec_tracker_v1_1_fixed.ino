// ============================================
// HELTEC TRACKER V1.1 - FIXED IP + OTAA ATTEMPT
// Fix 1: IP GPRS muncul langsung di TFT setelah connect
// Fix 2: OTAA attempt muncul di TFT (JOIN1/5, JOIN2/5 dst)
//        persis seperti program dosen (dokumen 1)
// ============================================
...
void setup()
{
  ...
  Serial.begin(115200);
  Serial.println("\n[BOOT] Heltec Tracker V1.1 Fixed (LoRa+WiFi+GPRS+IR+AntiCheat)");
...
void taskLora(void *pv)
{
  ...
  for (uint8_t a = 1; a <= MAX_JOIN_ATTEMPTS; a++) {
    ...
    if (s == RADIOLIB_LORAWAN_NEW_SESSION) {
      ...
      logSd("[LoRa] OTAA Join successful!");
      if (xSemaphoreTake(mLora, MTX) == pdTRUE) { g_lora.joined = true; xSemaphoreGive(mLora); }
      setLoraPage("JOINED", 0, 0, true);
      break;
    } else {
      // Tampilkan error join di Serial dan SD — seperti dokumen 1
      String errMsg = "[LoRa] Join failed attempt " + String(a) + ": " + decodeState(s);
  ...
  if (!g_lora.joined) {
    Serial.println("[LoRa] ❌ Max join attempts reached.");
    logSd("[LoRa] Max join attempts reached.");
...
// ============================================================
// taskGprsLoop — koneksi persisten + kirim terjadwal
// ============================================================
...
void taskGprsLoop(void *pv)
{
  ...
  if (strlen(GPRS_SIM_PIN) > 0 && !modem.simUnlock(GPRS_SIM_PIN)) {
    logSd("Failed to unlock SIM");
    Serial.println("Failed to unlock SIM");
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[5] = "Evt: SIM_FAIL"; xSemaphoreGive(mTft); }
    vTaskDelete(NULL);
  }
  ...
  for (int retry = 0; retry < 5; retry++) {
    // Tampilkan attempt di TFT dan Serial — seperti dokumen 1
    String retryMsg = "GPRS try " + String(retry + 1) + "/5";
    Serial.println("[GPRS] " + retryMsg);
    logSd(("[GPRS] " + retryMsg).c_str());
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      g_pg[2].row[1] = "Conn: NO";
      g_pg[2].row[2] = retryMsg;
    ...
    if (modem.gprsConnect(GPRS_APN, GPRS_USER, GPRS_PASS)) {
      ...
      // ✅ FIX: Update TFT langsung setelah connect — IP muncul segera
      if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
    ...
    logSd(("GPRS: Connect retry " + String(retry + 1) + " failed").c_str());
    vTaskDelay(pdMS_TO_TICKS(5000));
  }

  if (!gprsConnected) {
    logSd("GPRS: Failed to establish persistent connection");
    Serial.println("[GPRS] Failed persistent connection");
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      g_pg[2].row[1] = "Conn: NO";
      g_pg[2].row[2] = "IP: -";
  ...
  for (;;) {
    ...
    if (!gprsConnected || !modem.isGprsConnected()) {
      ...
      if (modem.gprsConnect(GPRS_APN, GPRS_USER, GPRS_PASS)) {
        ...
        // ✅ FIX: Update TFT langsung setelah reconnect — IP muncul segera
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
          g_pg[2].row[0] = "GPRS/DB Status";
          g_pg[2].row[1] = "Conn: YES";
          g_pg[2].row[2] = "IP: " + ip;
        ...
        gprsConnected = false;
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[2] = "IP: -"; g_pg[2].row[5] = "Evt: RCNN_FAIL"; xSemaphoreGive(mTft); }
        vTaskDelay(pdMS_TO_TICKS(2000));
        continue;
      }
    }

    // --- Kirim Data Terjadwal ---
    if (millis() - lastScheduledSend >= GPRS_INTERVAL_MS) {
      ...
      Serial.println("[GPRS] Scheduled: " + event);

      // Update TFT dengan IP terkini + hasil send
      String currentIp = "";
      ...
      if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
        g_pg[2].row[0] = "GPRS/DB Status";
        g_pg[2].row[1] = "Conn: YES";
        g_pg[2].row[2] = "IP: " + currentIp;
        g_pg[2].row[3] = "Last: " + String(millis() / 1000) + "s";
        String dispResp = responseLine.length() > 10 ? responseLine.substring(0, 10) : responseLine;
        g_pg[2].row[4] = "Resp: " + dispResp;
...
void sendImmediateGprsPayload(bool isCheat)
{
  ...
  if (gsmClient.connect(GPRS_HOST, GPRS_PORT)) {
    ...
    Serial.println("[GPRS] Immediate: " + event);

    // Ambil IP tersimpan untuk update TFT
    String currentIp = "";
    if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { currentIp = g_gprs.ip; xSemaphoreGive(mGprs); }
    ...
    Serial.println("[GPRS] Immediate: TCP connect failed");
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      g_pg[2].row[5] = "Evt: CONN_FAIL";
      xSemaphoreGive(mTft);
    }
