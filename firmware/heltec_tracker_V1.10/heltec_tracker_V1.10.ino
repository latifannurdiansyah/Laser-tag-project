...
// Tanpa mutex ini, sendImmediateGprsPayload (dari taskIr/taskAntiCheat)
...
void writeCsvHeader()
{
  ...
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    if (f) {
      if (!exists)
        f.println("sesi_id,tanggal,millis,jam,menit,detik,latitude,longitude,altitude_m,satellites,gps_valid,ir_status,cheat");
      f.close();
...
void writeGpsCsv()
{
  ...
  char row[160];
  snprintf(row, sizeof(row), "%s,%s,%lu,%02d,%02d,%02d,%.6f,%.6f,%.1f,%u,%s,%s,%u",
    g_csvSesiId, tglBuf,
    millis(), dh, m, s,
    lat, lng, alt, sat,
    locValid ? "1" : "0",
...
void sendThingSpeakGprs()
{
  ...
  if (xSemaphoreTake(mLora, MTX) == pdTRUE) { rssi = g_lora.rssi; snr = g_lora.snr; xSemaphoreGive(mLora); }
  if (xSemaphoreTake(mIr, MTX) == pdTRUE) { irAddr = g_ir.addr; irCmd = g_ir.cmd; xSemaphoreGive(mIr); }

  // FIX V1.6: Baca g_irHitPendingTS langsung (volatile, no mutex needed)
  uint8_t irFlag = g_irHitPendingTS ? 1 : 0;

  Serial.printf("[TS] irFlag=%u (g_irHitPendingTS=%d) addr=0x%04X cmd=0x%02X\n", irFlag, (int)g_irHitPendingTS, irAddr, irCmd);
  ...
  snprintf(jsonBody, sizeof(jsonBody),
    ...
    "\"field4\":%u,"     // sat
    "\"field5\":%d,"     // rssi
    "\"field6\":%d,"     // irFlag (1=HIT, 0=NONE) - numeric untuk React
    "\"field7\":%u,"     // cheat
    "\"field8\":%.1f,"   // snr
    "\"field9\":%u,"     // irAddr (address decimal)
    "\"field10\":%u}",   // irCmd (command decimal)
    TS_WRITE_KEY, lat, lng, alt, sat, rssi, irFlag, cheat, snr, irAddr, irCmd
  );
...
void taskGprsLoop(void *pv)
{
  ...
  for (;;) {
    ...
    // ThingSpeak — sendThingSpeakGprs sudah handle mGsmTcp di dalamnya
...
void sendImmediateGprsPayload(bool isCheat)
{
  ...
  // FIX V1.9: Minta mutex TCP — timeout 2 detik agar task IR tidak lama nunggu
