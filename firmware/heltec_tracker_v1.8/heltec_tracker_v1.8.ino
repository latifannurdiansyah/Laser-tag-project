...
// Sesi ID untuk CSV — diset sekali saat GPS pertama valid
// Format: HHMMSS (jam menit detik lokal saat fix pertama)
// Setiap power-on = nilai berbeda = sesi berbeda
char     g_csvSesiId[8]   = "000000"; // diisi taskGps saat fix pertama
bool     g_csvSesiIdSet   = false;    // flag: sudah diset atau belum
...
// Tidak akan di-reset oleh LoRa task atau task lain
// Hanya di-reset oleh sendThingSpeakGprs() saat ok==true
// ============================================
...
void sdSave(float, float, float, uint8_t, int16_t, float, const char*);
void writeCsvHeader();
void writeGpsCsv();
bool sdUpload();
...
void setup()
{
  ...
  tft.setCursor(10, 15); tft.print("Heltec Tracker V1.8");
  ...
  xTaskCreatePinnedToCore(taskGpsCsv, "GpsCsv", 4096, NULL, 1, NULL, 0);
}
  ...
  for (;;) {
    ...
    if (isCovered) {
      if (!wasCovered) { coverStart = millis(); wasCovered = true; }
        ...
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[5] = "Evt: CHEAT!"; xSemaphoreGive(mTft); }
        if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { g_gprs.lastEvt = "CHEAT"; xSemaphoreGive(mGprs); }
...
void taskGps(void *pv)
{
  for (;;) {
    ...
    // Catat sesi_id sekali saat GPS pertama valid (jam fix pertama)
    if (hasT && !g_csvSesiIdSet) {
      ...
      snprintf(g_csvSesiId, sizeof(g_csvSesiId), "%02d%02d%02d", dh, g_gps.m, g_gps.s);
      g_csvSesiIdSet = true;
...
void writeCsvHeader()
{
  ...
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    if (f) {
      if (!exists)
        f.println("sesi_id,tanggal,millis,jam,menit,detik,latitude,longitude,altitude_m,satellites,gps_valid");
      f.close();
      if (xSemaphoreTake(mCsv, MTX) == pdTRUE) { g_csv.headerOk = true; xSemaphoreGive(mCsv); }
    }
    xSemaphoreGive(mSpi);
  }
}
...
void writeGpsCsv()
{
  ...
  if (xSemaphoreTake(mGps, MTX) == pdTRUE) {
    ...
    // Ambil tanggal dari GPS object langsung (TinyGPS++ global)
    if (GPS.date.isValid()) {
      gday  = GPS.date.day();
      gmon  = GPS.date.month();
      gyear = GPS.date.year();
  ...
  // Format tanggal DDMMYYYY, pakai 00000000 jika GPS date belum valid
  char tglBuf[10];
  if (gyear > 2000)
    snprintf(tglBuf, sizeof(tglBuf), "%02u%02u%04u", gday, gmon, gyear);
  else
    snprintf(tglBuf, sizeof(tglBuf), "00000000");

  // Sesi ID: pakai g_csvSesiId (diset taskGps saat fix pertama)
  char row[140];
  snprintf(row, sizeof(row), "%s,%s,%lu,%02d,%02d,%02d,%.6f,%.6f,%.1f,%u,%s",
    g_csvSesiId, tglBuf,
    millis(), dh, m, s,
    lat, lng, alt, sat,
    locValid ? "1" : "0");
  ...
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    if (f) {
      ...
      if (xSemaphoreTake(mCsv, MTX) == pdTRUE) { g_csv.rowCount++; xSemaphoreGive(mCsv); }
    }
    xSemaphoreGive(mSpi);
  }
}
...
void taskLora(void *pv)
{
  ...
  if (radio.begin() != RADIOLIB_ERR_NONE) { setLoraPage("RADIO_FAIL", 0, 0, false); vTaskDelete(NULL); }
  ...
  for (uint8_t a = 1; a <= MAX_JOIN_ATTEMPTS; a++) {
    String attemptMsg = "OTAA " + String(a) + "/" + String(MAX_JOIN_ATTEMPTS);
  ...
  if (!g_lora.joined) { setLoraPage("NO_JOIN", 0, 0, false); vTaskDelete(NULL); }
  ...
  for (;;) {
    if (irSend || millis() - tUp >= UPLINK_INTERVAL_MS) {
      ...
      if (s < RADIOLIB_ERR_NONE) { ev = "TX_FAIL"; }
        ...
        snr  = radio.getSNR();
        rssi = radio.getRSSI();
        // LoRa task hanya reset g_ir.got — g_irHitPendingTS TIDAK di-reset di sini
...
// FIX V1.6:
//   - irFlag pakai g_irHitPendingTS (bukan g_ir.got)
//   - g_irHitPendingTS di-reset HANYA jika ok == true
//   - Sehingga status HIT tidak hilang meski g_ir.got
//     sudah di-reset oleh LoRa task
// ThingHTTP body: field6 tanpa kutip (integer 1/0)
// ============================================
#if ENABLE_GPRS

void sendThingSpeakGprs()
{
  if (!modem.isGprsConnected()) { logSd("[TS] GPRS not connected"); return; }

  float lat = 0, lng = 0, alt = 0;
  uint8_t sat = 0; int16_t rssi = 0; float snr = 0;
  uint8_t cheat = g_cheatDetected ? 1 : 0;

  if (xSemaphoreTake(mGps,  MTX) == pdTRUE) { lat = g_gps.lat; lng = g_gps.lng; alt = g_gps.alt; sat = g_gps.sat; xSemaphoreGive(mGps); }
  if (xSemaphoreTake(mLora, MTX) == pdTRUE) { rssi = g_lora.rssi; snr = g_lora.snr; xSemaphoreGive(mLora); }

  // FIX V1.6: Baca g_irHitPendingTS langsung (volatile, no mutex needed)
  uint8_t irFlag = g_irHitPendingTS ? 1 : 0;

  Serial.printf("[TS] irFlag=%u (g_irHitPendingTS=%d)\n", irFlag, (int)g_irHitPendingTS);
  ...
  if (gsmClientTS.connect(TS_HOST, TS_PORT)) {
    ...
    gsmClientTS.print("Content-Length: "); gsmClientTS.print(bodyLen);
    gsmClientTS.print("\r\nConnection: close\r\n\r\n");
    gsmClientTS.print(jsonBody);

    unsigned long timeout = millis();
    while (gsmClientTS.available() == 0) {
      if (millis() - timeout > 8000) { Serial.println("[TS] Timeout"); break; }
      vTaskDelay(pdMS_TO_TICKS(10));
    }
    String response = "";
    while (gsmClientTS.available()) response += (char)gsmClientTS.read();
    gsmClientTS.stop();

    if (response.startsWith("HTTP/")) {
      int spaceIdx = response.indexOf(' ');
      if (spaceIdx > 0) respCode = response.substring(spaceIdx + 1, spaceIdx + 4).toInt();
    }
    ok = (respCode == 200 && response.indexOf("entry_id") >= 0);

    Serial.printf("[TS] Response code: %d\n", respCode);
    Serial.println("[TS] Body: " + response.substring(response.lastIndexOf("\r\n\r\n") + 4, response.length()));
  } else {
  ...
  if (xSemaphoreTake(mTs, MTX) == pdTRUE) {
    g_ts.lastOk = ok; g_ts.lastCode = respCode; g_ts.lastSentMs = millis();
    if (ok) g_ts.okCount++; else g_ts.failCount++;
    xSemaphoreGive(mTs);
  }

  char logMsg[80];
  snprintf(logMsg, 80, "[TS] %s code:%d lat:%.4f lng:%.4f ir:%u", ok ? "OK" : "FAIL", respCode, lat, lng, irFlag);
...
void buildDataPayload(Payload &payload)
{
  payload.addr_id = 0x12345678; payload.sub_id = random(0, 256); payload.status = 1;
  if (xSemaphoreTake(mGps, MTX) == pdTRUE) { payload.lat = g_gps.lat; payload.lng = g_gps.lng; payload.alt = g_gps.alt; xSemaphoreGive(mGps); }
  else { payload.lat = payload.lng = payload.alt = 0.0f; }
  if (xSemaphoreTake(mIr, MTX) == pdTRUE) { payload.shooter_addr = g_ir.got ? g_ir.addr : 0; payload.shooter_sub = g_ir.got ? g_ir.cmd : 0; payload.sub_id = g_ir.got ? g_ir.cmd : 0; xSemaphoreGive(mIr); }
...
void taskGprsLoop(void *pv)
{
  if (!modem.begin()) {
    ...
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[5] = "Evt: NO_MODEM"; xSemaphoreGive(mTft); }
    vTaskDelete(NULL);
  }

  if (strlen(GPRS_SIM_PIN) > 0 && !modem.simUnlock(GPRS_SIM_PIN)) {
    logSd("SIM unlock failed");
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[5] = "Evt: SIM_FAIL"; xSemaphoreGive(mTft); }
    vTaskDelete(NULL);
  }
  ...
  for (int retry = 0; retry < 5; retry++) {
    ...
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[2] = msg; g_pg[2].row[5] = "Evt: CONNECTING";
      xSemaphoreGive(mTft);
    }
    if (modem.gprsConnect(GPRS_APN, GPRS_USER, GPRS_PASS)) {
      ...
      if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
        g_pg[2].row[0] = "GPRS Status"; g_pg[2].row[1] = "Conn: YES";
        g_pg[2].row[2] = "IP: " + ip; g_pg[2].row[5] = "Evt: CONNECTED";
        xSemaphoreGive(mTft);
      }
  ...
  if (!gprsConnected) {
    ...
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[2] = "IP: -"; g_pg[2].row[5] = "Evt: CONN_FAIL";
      xSemaphoreGive(mTft);
    }
  }
  ...
  for (;;) {
    if (!gprsConnected || !modem.isGprsConnected()) {
      ...
      if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[5] = "Evt: RECONNECTING"; xSemaphoreGive(mTft); }
      if (modem.gprsConnect(GPRS_APN, GPRS_USER, GPRS_PASS)) {
        String ip = modem.getLocalIP();
        gprsConnected = true;
        Serial.println("[GPRS] Reconnected! IP=" + ip);
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
          g_pg[2].row[1] = "Conn: YES"; g_pg[2].row[2] = "IP: " + ip; g_pg[2].row[5] = "Evt: RECONNECTED";
          xSemaphoreGive(mTft);
        }
        if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { g_gprs.connected = true; g_gprs.ip = ip; xSemaphoreGive(mGprs); }
      } else {
        gprsConnected = false;
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[1] = "Conn: NO"; g_pg[2].row[5] = "Evt: RCNN_FAIL"; xSemaphoreGive(mTft); }
    ...
    if (millis() - lastDbSend >= GPRS_INTERVAL_MS) {
      ...
      if (gsmClient.connect(GPRS_HOST, GPRS_PORT)) {
        ...
        unsigned long timeout = millis();
        while (gsmClient.available() == 0) { if (millis() - timeout > 5000) break; }
        if (gsmClient.available()) {
          String response = gsmClient.readString();
          event = (response.indexOf("OK") >= 0) ? "DB_OK" : "DB_FAIL";
      ...
      String currentIp = "";
      if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { currentIp = g_gprs.ip; xSemaphoreGive(mGprs); }
      if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
        g_pg[2].row[3] = "Last: " + String(millis() / 1000) + "s";
        g_pg[2].row[4] = "Resp: " + (responseLine.length() > 10 ? responseLine.substring(0, 10) : responseLine);
        g_pg[2].row[5] = "Evt: " + event;
        xSemaphoreGive(mTft);
      }
      if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { g_gprs.lastEvt = event; g_gprs.lastResp = responseLine; xSemaphoreGive(mGprs); }
      lastDbSend = millis();
    }
...
void sendImmediateGprsPayload(bool isCheat)
{
  if (!modem.isGprsConnected()) {
    ...
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[5] = "Evt: NO_GPRS"; xSemaphoreGive(mTft); }
    return;
  }
  Payload payload; buildDataPayload(payload);
  ...
  if (gsmClient.connect(GPRS_HOST, GPRS_PORT)) {
    ...
    gsmClient.stop();
    String event = (response.indexOf("OK") >= 0) ? (isCheat ? "CHEAT_OK" : "IR_DB_OK") : (isCheat ? "CHEAT_FAIL" : "IR_DB_FAIL");
    logSd(("IMM GPRS: " + event).c_str());
    String currentIp = "";
    if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { currentIp = g_gprs.ip; xSemaphoreGive(mGprs); }
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[2] = "IP: " + currentIp; g_pg[2].row[5] = "Evt: " + event; xSemaphoreGive(mTft); }
    if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { g_gprs.lastEvt = event; xSemaphoreGive(mGprs); }
  } else {
    logSd("IMM GPRS: TCP failed");
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) { g_pg[2].row[5] = "Evt: CONN_FAIL"; xSemaphoreGive(mTft); }
  }
}
#endif
...
//           Flag ini tidak di-reset oleh siapapun
//           kecuali sendThingSpeakGprs() saat ok==true
// ============================================
void taskIr(void *pv)
{
  for (;;) {
    if (IrReceiver.decode()) {
      if (IrReceiver.decodedIRData.protocol == NEC) {
        ...
        if (xSemaphoreTake(mIr, MTX) == pdTRUE) {
          ...
          g_ir.hitCount++; irPend = true; irAddr = a; irCmd = c; irSend = true;
          xSemaphoreGive(mIr);
        }
        // FIX V1.6: Set flag persistent untuk ThingSpeak
        g_irHitPendingTS = true;
        Serial.println("[IR] g_irHitPendingTS = true (HIT detected)");
        ...
        #if ENABLE_GPRS
        for (uint8_t i = 0; i < IR_IMMEDIATE_SEND; i++) { sendImmediateGprsPayload(false); vTaskDelay(pdMS_TO_TICKS(200)); }
        #endif
      }
      IrReceiver.resume();
    }
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
      if (xSemaphoreTake(mIr, MTX) == pdTRUE) {
        if (!g_ir.got) { g_pg[3].row[1] = "Shooter: -"; g_pg[3].row[2] = "Sub: -"; g_pg[3].row[3] = "Loc: NONE"; g_pg[3].row[4] = "Time: --:--"; }
        xSemaphoreGive(mIr);
      }
      xSemaphoreGive(mTft);
    }
...
void sdSave(float lat, float lng, float alt, uint8_t sat, int16_t rssi, float snr, const char* ir)
{
  #if ENABLE_SD
  if (!sdOK) return;
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(100)) == pdTRUE) {
    ...
    if (f) {
      char ln[200]; snprintf(ln, 200, "%lu,%.6f,%.6f,%.1f,%u,%d,%.1f,%s", millis(), lat, lng, alt, sat, rssi, snr, ir);
...
bool sdUpload()
{
  ...
  if (xSemaphoreTake(mSpi, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    while (f.available()) {
      ...
      int c[7], cc = 0; for (size_t i = 0; i < ln.length() && cc < 7; i++) if (ln.charAt(i) == ',') c[cc++] = i;
      if (cc < 6) continue;
      ...
      int16_t rssi = ln.substring(c[4]+1, c[5]).toInt(); float snr = ln.substring(c[5]+1, c[6]).toFloat();
      ...
      char pl[400]; snprintf(pl, 400,
        "{\"source\":\"wifi_sd\",\"id\":\"%s\",\"lat\":%.6f,\"lng\":%.6f,\"alt\":%.1f,\"irStatus\":\"%s\",\"satellites\":%u,\"rssi\":%d,\"snr\":%.1f}",
