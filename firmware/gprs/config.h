...
#define SERVER "gps-gprs-79fd6-default-rtdb.asia-southeast1.firebasedatabase.app"
#define RESOURCE "/gps_tracking.json"
#define RESOURCE_IR "/ir_hits.json"
#define SERVER_PORT 80
#define FIREBASE_SECRET "P2YqqBER30Y3Oz21f83MaQUs5B06C58t8PsDI8fu"
...
//     float lat, lng, alt;
//     uint8_t irHitLocation;
//     uint8_t local_hour, local_minute, local_second; // ← Include local time in payload
// };

// config.h — MUST be exactly this:
struct __attribute__((packed)) DataPayload {
    uint32_t address_id;
    uint8_t sub_address_id;
    uint32_t shooter_address_id;
    ...
    uint8_t status;
    float lat, lng, alt;
    uint8_t irHitLocation;   // ← total 24 bytes
};
...
// extern SemaphoreHandle_t xIrSendMutex;  // Disabled - IR not used
