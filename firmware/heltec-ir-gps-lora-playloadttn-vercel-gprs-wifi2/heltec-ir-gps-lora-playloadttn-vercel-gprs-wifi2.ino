...
// Endpoint khusus cheat event (GPRS)
...
// ANTI-CHEAT CONFIG
// ============================================
#define IR_AMBIENT_PIN        2          // Photodiode/komparator ambient IR
#define CHEAT_DETECTION_MS    5000       // Durasi cover sebelum trigger cheat
#define CHEAT_CHECK_INTERVAL  100        // Polling interval (ms)
#define CHEAT_ALERT_DURATION  10000      // Durasi banner merah di TFT (ms)
#define IR_IMMEDIATE_SEND     2          // Jumlah kirim langsung saat cheat/IR hit
...
#define GPRS_RX_PIN    16   // ← Dipindah dari 18 (konflik LED_BUILTIN)
...
const uint32_t GPRS_INTERVAL_MS   = 30000;  // Scheduled GPRS 30 detik
...
// Background hitam seperti program referensi
// ============================================
#define TFT_W      160
#define TFT_H       80
#define TFT_PAGES    4
...
struct __attribute__((packed)) Payload {
    ...
    int16_t  rssi;
    int8_t   snr;
    uint8_t  cheatDetected;  // ← NEW: 1=cheat, 0=normal
};
...
// 4 halaman TFT — background hitam, teks putih
// Halaman: 0=LoRaWAN, 1=GPS, 2=GPRS, 3=IR
Page g_pg[TFT_PAGES] = {
    { {"LoRaWAN Status", "Event: IDLE", "RSSI: -", "SNR: -", "Joined: NO", "-"} },
    { {"GPS Status", "Lat: -", "Long: -", "Alt: -", "Sat: -", "--:--:--"} },
    { {"GPRS Status", "Conn: NO", "IP: -", "Resp: -", "Evt: IDLE", "-"} },
...
void gprsPost(float lat, float lng, bool isCheat);
void gprsPostImmediate(bool isCheat);
...
void setup()
{
    ...
    // CPU 0: TFT (prioritas tertinggi agar render lancar)
...
// ============================================
// TFT TASK — 4 HALAMAN, BACKGROUND HITAM
// ============================================
void taskTft(void *pv)
{
    ...
    for (;;)
    {
        ...
        // Cek status koneksi
        bool lOk = false, gOk = false, wOk = false, sOk = sdOK;
        ...
        // Cek cheat banner
        bool showCheat = (millis() < g_cheatAlertExpiry);

        // ─── Render ke framebuffer ───────────────────────────────────────
        fb.fillScreen(ST7735_BLACK);
        fb.setTextSize(1);

        // STATUS BAR atas: L:OK G:OK W:OK S:OK  [n/4]
        fb.setCursor(0, 0);
        ...
        fb.setTextColor(ST7735_YELLOW);
        fb.setCursor(TFT_W - 24, 0);
        fb.printf("[%d/4]", pg + 1);

        // Garis pemisah
        fb.drawFastHLine(0, 9, TFT_W, 0x39C7); // abu-abu gelap

        // Judul halaman — warna berbeda per halaman
        // 0=LoRa: cyan, 1=GPS: kuning, 2=GPRS: hijau, 3=IR: oranye
        const uint16_t titleColor[TFT_PAGES] = {
            ST7735_CYAN,    // LoRaWAN
            ST7735_YELLOW,  // GPS
            0x07E0,         // GPRS (green)
            0xFD20          // IR (orange)
        };
        fb.setTextColor(titleColor[pg]);
        fb.setCursor(LEFT_M, TOP_M + LINE_H);
        fb.print(snap.row[0]);

        // Baris data 1–5 (putih)
        fb.setTextColor(ST7735_WHITE);
        for (int i = 1; i < TFT_ROWS; i++) {
            int y = TOP_M + (i + 1) * LINE_H;
            if (y + LINE_H > TFT_H - (showCheat ? 10 : 0)) break;
            fb.setCursor(LEFT_M, y);
            fb.print(snap.row[i]);
        }

        // CHEAT BANNER — baris bawah merah berkedip
        if (showCheat) {
            fb.fillRect(0, TFT_H - 10, TFT_W, 10, ST7735_RED);
            fb.setTextColor(ST7735_WHITE, ST7735_RED);
            fb.setCursor(22, TFT_H - 9);
...
// ============================================
// ANTI-CHEAT TASK (CPU 1)
// Deteksi IR ambient tertutup > CHEAT_DETECTION_MS
// ============================================
    ...
    for (;;) {
        ...
        if (isCovered) {
            if (!wasCovered) {
                ...
                // Kirim immediate GPRS (cheat endpoint)
                for (uint8_t i = 0; i < IR_IMMEDIATE_SEND; i++) {
                    gprsPostImmediate(true);
...
void taskLora(void *pv)
{
    ...
    auto setLoraPage = [](const char* ev, int16_t rssi, float snr, bool joined) {
        if (xSemaphoreTake(mTft, pdMS_TO_TICKS(30)) == pdTRUE) {
            ...
            g_pg[0].row[1] = String("Event: ") + ev;
            g_pg[0].row[2] = "RSSI: " + String(rssi) + " dBm";
            g_pg[0].row[3] = "SNR: " + String(snr, 1) + " dB";
            g_pg[0].row[4] = joined ? "Joined: YES" : "Joined: NO";
            g_pg[0].row[5] = "-";
            xSemaphoreGive(mTft);
        }
    };
...
void initGprsHardware()
{
    ...
    for (int r = 0; r < 5 && !conn; r++) {
        ...
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
            g_pg[2].row[5] = "Evt: TRY " + String(r + 1) + "/5";
            xSemaphoreGive(mTft);
        }
        if (modem.gprsConnect(GPRS_APN, GPRS_USER, GPRS_PASS)) {
            ...
            Serial.printf("[GPRS] Attempt %d failed\n", r + 1);
...
// ============================================
// GPRS LOOP TASK (CPU 1)
// Scheduled send setiap GPRS_INTERVAL_MS
// Update TFT halaman 2 (index 2)
// ============================================
...
// ============================================
// gprsPost — Scheduled & IR immediate send
// isCheat=true → kirim ke endpoint cheat
// ============================================
void gprsPost(float lat, float lng, bool isCheat)
{
    ...
    if (xSemaphoreTake(mIr, MTX) == pdTRUE) {
        if (g_ir.got) { ia = g_ir.addr; ic = g_ir.cmd; hitStatus = "HIT"; }
        xSemaphoreGive(mIr);
    }
    if (xSemaphoreTake(mGps,  MTX) == pdTRUE) { alt = g_gps.alt; sat = g_gps.sat; xSemaphoreGive(mGps); }
    if (xSemaphoreTake(mLora, MTX) == pdTRUE) { rssi = g_lora.rssi; snr = g_lora.snr; xSemaphoreGive(mLora); }
    ...
    snprintf(pl, sizeof(pl),
        ...
        "\"cheatDetected\":%d}",
        DEVICE_ID, lat, lng, alt, sat, rssi, snr,
        ia, ic, hitStatus, isCheat ? 1 : 0);

    const char* host = "simpera.teknoklop.com";
    const char* path = isCheat ? "/lasertag/api/cheat_event.php" : "/lasertag/api/track.php";
    int port = 10000;

    String event = isCheat ? "CHEAT_FAIL" : "DB_FAIL";
    String resp  = "-";
    ...
    if (gsmClient.connect(host, port)) {
        ...
        // Cari baris terakhir (JSON response body)
        int bodyIdx = raw.indexOf("\r\n\r\n");
        if (bodyIdx >= 0) resp = raw.substring(bodyIdx + 4, bodyIdx + 4 + 20);
        else resp = raw.substring(0, min(20, (int)raw.length()));
        resp.trim();

        bool ok = (raw.indexOf("200 OK") >= 0) || (raw.indexOf("201") >= 0) || (raw.indexOf("\"ok\"") >= 0);
        event = isCheat ? (ok ? "CHEAT_OK" : "CHEAT_FAIL") : (ok ? "DB_OK" : "DB_FAIL");

        Serial.printf("[GPRS] %s → %s | Resp: %s\n", isCheat ? "CHEAT" : "Sched", event.c_str(), resp.c_str());
        logSd(("[GPRS] " + event + " | " + resp).c_str());
    } else {
        event = "CONN_FAIL";
        Serial.printf("[GPRS] Connect to %s:%d FAILED\n", host, port);
        logSd("[GPRS] TCP connect failed");
    }

    // Update TFT halaman GPRS
    if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
        g_pg[2].row[0] = "GPRS Status";
        g_pg[2].row[1] = "Conn: YES";
        bool gOk = false;
        if (xSemaphoreTake(mGprs, MTX) == pdTRUE) { gOk = g_gprs.connected; xSemaphoreGive(mGprs); }
        g_pg[2].row[2] = gOk ? ("IP: " + g_gprs.ip.substring(0, min(14, (int)g_gprs.ip.length()))) : "IP: -";
        g_pg[2].row[3] = "Resp: " + (resp.length() > 10 ? resp.substring(0, 10) : resp);
        g_pg[2].row[4] = "Type: " + String(isCheat ? "CHEAT" : "SCHED");
        g_pg[2].row[5] = "Evt: " + event;
        xSemaphoreGive(mTft);
    }

    if (xSemaphoreTake(mGprs, MTX) == pdTRUE) {
        g_gprs.lastEvt  = event;
        g_gprs.lastResp = resp;
        xSemaphoreGive(mGprs);
    }
}

// gprsPostImmediate: ambil lat/lng terbaru lalu panggil gprsPost
void gprsPostImmediate(bool isCheat)
{
    float lat = 0, lng = 0;
    if (xSemaphoreTake(mGps, MTX) == pdTRUE) { lat = g_gps.lat; lng = g_gps.lng; xSemaphoreGive(mGps); }
    gprsPost(lat, lng, isCheat);
}
#endif
    ...
    for (;;) {
        if (IrReceiver.decode()) {
            if (IrReceiver.decodedIRData.protocol == NEC) {
                ...
                // Kirim immediate GPRS saat IR hit
#if ENABLE_GPRS
                for (uint8_t i = 0; i < IR_IMMEDIATE_SEND; i++) {
                    gprsPostImmediate(false);
                    vTaskDelay(pdMS_TO_TICKS(200));
                }
#endif
            }
        ...
        // Jaga TFT page IR agar selalu fresh (update timestamp "ago")
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
            if (xSemaphoreTake(mIr, MTX) == pdTRUE) {
                if (!g_ir.got) {
