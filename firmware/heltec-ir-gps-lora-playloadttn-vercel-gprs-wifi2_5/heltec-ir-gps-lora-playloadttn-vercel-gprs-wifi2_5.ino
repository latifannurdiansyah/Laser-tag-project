...
#define GPRS_SIM_PIN       ""          // Kosong = tidak ada PIN

// Server GPRS (identik referensi, pakai define terpisah)
...
#endif
#if ENABLE_GPRS
    initGprsHardware();   // Reset & init UART SIM900A
...
// ============================================
// TFT TASK — COPY PASTE DARI REFERENSI
// heltec_lora_sdcard_tft_gps_gprs_miles_irremote_anti_cheat.ino
// 4 halaman: [0]LoRaWAN [1]GPS [2]GPRS [3]IR
// Background hitam, teks putih, banner merah CHEAT RECORDED
// ============================================
void taskTft(void *pv)
{
    vTaskDelay(pdMS_TO_TICKS(2000)); // Splash 2 detik agar SPI bebas
    ...
    for (;;)
    {
        // Auto-slide setiap PAGE_MS — identik referensi
        if (millis() - pageSwitch >= PAGE_MS) {
            currentPage = (currentPage + 1) % TFT_PAGES;
            pageSwitch = millis();
        }

        // Mutex scope mencakup render + cheat check — identik referensi
        if (xSemaphoreTake(mTft, pdMS_TO_TICKS(100)) == pdTRUE) {

            // Background hitam — identik referensi
            fb.fillScreen(ST7735_BLACK);
            fb.setTextColor(ST7735_WHITE);
            fb.setTextSize(1);

            // Render 6 baris halaman aktif — identik referensi
            // rows[0] = judul halaman, rows[1..5] = data
            ...
            // Cek cheat banner — identik referensi
            bool showCheatBanner = (millis() < g_cheatAlertExpiry);

            if (showCheatBanner) {
                // Identik referensi: fillRect(0,70,160,10), teks cursor(20,71)
            ...
            // Flush ke layar — SPI dilindungi mSpi (SD pakai SPI yang sama)
    ...
    for (;;) {
        ...
        if (isCovered) {
            if (!wasCovered) {
                ...
                // Kirim immediate GPRS (cheat endpoint) — sama seperti referensi
...
void taskGps(void *pv)
{
    for (;;) {
        ...
        if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
            // UTC offset dari longitude GPS — identik referensi (getUtcOffsetFromLongitude)
            int8_t dh = 0;
            if (hasT) {
                float lon = g_gps.locValid ? g_gps.lng : 107.0f; // default WIB jika belum ada GPS
...
// ============================================
// GPRS SIM900A — COPY PASTE DARI REFERENSI
// heltec_lora_sdcard_tft_gps_gprs_miles_irremote_anti_cheat.ino
// Disesuaikan nama variabel lokal program ini
// ============================================
#if ENABLE_GPRS

// ── buildDataPayload: kumpulkan semua state ke struct Payload ─────────────────
// Referensi: buildDataPayload(DataPayload &payload)
void buildDataPayload(Payload &payload)
{
...
// ── gprsTask: init modem + koneksi + scheduled send loop ─────────────────────
// COPY PASTE dari referensi: gprsTask(void *pv)
// Catatan: referensi init modem di dalam task (sebelum loop),
//          program ini memisahkan ke initGprsHardware() + taskGprsLoop()
//          agar reset hardware (GPRS_RST_PIN) bisa pakai delay() murni
//          sebelum scheduler FreeRTOS jalan. Logika identik.

void initGprsHardware()
{
    // Reset hardware SIM900A — pakai delay() murni (sebelum scheduler)
...
{
    // ── IDENTIK dengan awal gprsTask() referensi ──────────────────────────────
    if (!modem.begin()) {
    ...
    // SIM PIN unlock — identik referensi
    ...
    // Coba koneksi GPRS — identik referensi (5 retry, delay 5000)
    ...
    // ── LOOP IDENTIK dengan for(;;) di gprsTask() referensi ──────────────────
    uint32_t lastScheduledSend = 0;
    for (;;) {
        // Cek & reconnect — copy paste referensi
        ...
        // Scheduled send — identik referensi
        if (millis() - lastScheduledSend >= GPRS_INTERVAL_MS) {
            ...
            // Identik referensi: connect → POST octet-stream → baca response → cari "OK"
            ...
            // Update TFT — identik referensi: rows[1]=Conn, rows[2]=IP, rows[4]=Resp, rows[5]=Evt
            if (xSemaphoreTake(mTft, MTX) == pdTRUE) {
...
// ── sendImmediateGprsPayload — COPY PASTE dari referensi ─────────────────────
// Parameter isCheat: true → kirim ke GPRS_CHEAT_PATH, false → GPRS_PATH
void sendImmediateGprsPayload(bool isCheat)
{
    ...
    if (gsmClient.connect(GPRS_HOST, GPRS_PORT)) {
        ...
        // Identik referensi: tunggu response 5 detik, char by char
        String response = "";
    ...
    for (;;) {
        if (IrReceiver.decode()) {
            if (IrReceiver.decodedIRData.protocol == NEC) {
                ...
                // Kirim immediate GPRS saat IR hit — sama seperti referensi
#if ENABLE_GPRS
                for (uint8_t i = 0; i < IR_IMMEDIATE_SEND; i++) {
